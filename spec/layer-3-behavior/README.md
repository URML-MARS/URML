...
The cross-robot collision check is geometric, modeled on UTM ([RFC-0287](../../docs/rfcs/0287-utm-strategic-deconfliction.md)): a `parallel` is one time window, and a **`barrier` is temporal deconfliction** — it ends a window, so volumes on either side of it are separated in time and never compared. Within a window, two members are safe if their volumes are separated laterally, vertically (altitude for air, depth for water), or by medium. Large-scale, multi-site fleet *traffic* management is out of scope and composes with Open-RMF ([RFC-0053](../../docs/rfcs/0053-open-rmf-multirobot-integration.md)) rather than living in URML.
