...
Combining and synchronizing fleet behavior therefore reuses the operators this layer already owns: `sequence` / `parallel` of `on:`-scoped subtrees, with `barrier:` between the steps that must not overlap. The teeth live in the validator's fleet pass, which rejects an `on:`/`barrier:` naming an undeclared member, a primitive a member's manifest cannot satisfy, two members whose operational volumes conflict concurrently in one `parallel`, and a `barrier` member lacking the `peer_link` connectivity role.

The cross-robot collision check is geometric, modeled on UTM ([RFC-0291](../../docs/rfcs/0291-utm-strategic-deconfliction.md)): a `parallel` is one time window, and a **`barrier` is temporal deconfliction** — it ends a window, so volumes on either side of it are separated in time and never compared. Within a window, two members are safe if their volumes are separated laterally, vertically (altitude for air, depth for water), or by medium. With the frame-transform graph ([RFC-0290](../../docs/rfcs/0290-frame-transform-graph.md)) the comparison is **cross-frame**: a drone's `agl` target and a rover's `site` target resolve into one declared world before they are compared. Large-scale, multi-site fleet *traffic* management is out of scope and composes with Open-RMF ([RFC-0053](../../docs/rfcs/0053-open-rmf-multirobot-integration.md)) rather than living in URML.
