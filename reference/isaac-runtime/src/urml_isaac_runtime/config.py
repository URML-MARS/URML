"""Deployment-side configuration for ``IsaacAdapter``.

A Isaac simulation needs to know:
...
``isaacsim_adapter.yaml`` alongside the URML artifacts.
"""
...
def load_isaac_config(path: str | Path) -> IsaacConfig:
    """Parse a ``isaacsim_adapter.yaml`` file into a ``IsaacConfig``."""
    p = Path(path)
    with p.open(encoding="utf-8") as fh:
        data = yaml.safe_load(fh) or {}
    if not isinstance(data, dict):
        raise ValueError(f"isaacsim-config file {p} did not contain a YAML mapping at the top level.")
    return IsaacConfig.model_validate(data)
