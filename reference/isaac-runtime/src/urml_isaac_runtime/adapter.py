"""IsaacAdapter — NVIDIA Isaac Sim physics simulator, via the ``isaacsim`` wheel.

Isaac is the official Apache-2.0 simulator with pure-Python bindings
over a bundled engine: headless, offline, no ROS. This adapter mirrors
:class:`BlueRovAdapter` / :class:`PX4Adapter`: lazy ``isaacsim``, **no
...
- ``move_to`` / ``hover`` → write the configured control target onto
  ...
  (location → ``ctrl`` via :class:`IsaacConfig`); ``hover`` is a
...
from urml_isaac_runtime.config import ControlTarget, IsaacConfig, load_isaac_config
...
_NOT_SUPPORTED = (
    "not_supported_in_base_isaac_sim: a bare NVIDIA Isaac Sim physics model has no {capability}. "
...
def _require_isaacsim() -> Any:
    """Lazy-import isaacsim with a clear error when missing."""
    try:
        import isaacsim  # type: ignore[import-not-found,unused-ignore]
    except ImportError as exc:
        raise RuntimeError(
            "isaacsim is not installed. IsaacAdapter requires the [sim] extra.\n"
            "  Install with: pip install urml-isaac-runtime[sim]"
        ) from exc
    return isaacsim


class IsaacAdapter:
    """Isaac Sim/Lab simulator adapter implementing the URML ROSAdapter Protocol."""

    BRAND = "isaacsim"

    def __init__(self, config: IsaacConfig | None = None) -> None:
        self._isaacsim = _require_isaacsim()
        self._config = config or IsaacConfig()
    ...
    def _sim(self) -> tuple[Any, Any]:
        ...
        if not self._config.model_path:
            raise RuntimeError(
                "model_path not configured: set IsaacConfig.model_path to an "
                "MJCF/XML model file in isaacsim_adapter.yaml."
            )
        model = self._isaacsim.IsaacModel.from_xml_path(self._config.model_path)
        data = self._isaacsim.IsaacData(model)
    ...
    def _step(self, n: int) -> None:
        ...
        for _ in range(max(1, n)):
            self._isaacsim.sim_step(model, data)

    def close(self) -> None:
        self._closed = True

    def __enter__(self) -> IsaacAdapter:
        return self
        ...
        if location is not None:
            ...
            if tgt is None:
                return NavigationResult(
                    ...
                    "manifest but not mapped to a control target in isaacsim_adapter.yaml.",
                )
            ctrl = list(tgt.ctrl)
        else:
