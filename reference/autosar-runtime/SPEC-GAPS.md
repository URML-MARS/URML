...
# SPEC-GAPS — urml-autosar-runtime

Per the spec-gap protocol (RFC-0014): this runtime is built strictly
against the frozen substrate Protocol. Anything the AUTOSAR Adaptive
...
- **RFC-0015 — ara::com Method / PLC-method invocation.** AUTOSAR Adaptive
  Robotics cells routinely expose "run named ara::com Method P with
...
- **RFC-0016 — Real-time / cyclic manifest block.** A fieldbus/AUTOSAR Adaptive
  ...
  in `autosar_adapter.yaml`, not URML-declared).
...
- **Station services** (e.g. `swap_tool`, `park`) → `send_docking_goal`
  ...
  unchanged; `service_to_method` in `AutosarConfig` maps it.
...
`ara` is **LGPL-3.0**. In-repo code is Apache-2.0 and depends on it
**only** via the optional `[autosar]` extra, never imported at module
...
maintainer to ratify on this PR. Fallback if rejected: gate the AUTOSAR Adaptive
