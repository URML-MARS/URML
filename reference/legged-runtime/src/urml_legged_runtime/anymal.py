"""AnymalAdapter — ANYbotics ANYmal, a ROS 2 inspection quadruped.

Unlike Spot (its own gRPC SDK), ANYmal speaks ROS 2, so this adapter
composes the ROS 2 runtime's :class:`RclpyAdapter` — the same
single-sourced-substrate pattern the industrial-arm runtime uses for
ABB/FANUC/KUKA/YASKAWA. The per-brand surface is real (distinct class,
brand-scoped not-supported tag); the ROS 2 plumbing is not duplicated.

A bare ANYmal is a four-legged inspection platform: it navigates
(``move_to`` / ``hover``), dwells, measures (inspection-payload
sensors), waits on cell/site signals, and reports. It has no arm and no
flight. Those primitives return ``not_supported_on_quadruped[anymal]``
rather than raising — the same pattern PX4Adapter and the industrial
arms use. A cell pairing ANYmal with a manipulation or vision payload
dispatches through a companion adapter (the drone-stack pattern).
...
__all__ = ["AnymalAdapter"]

_NOT_SUPPORTED = (
    "not_supported_on_quadruped[anymal]: a bare ANYmal inspection robot has no "
    "{capability}. Pair it with a companion adapter (manipulation / vision "
    "payload) for full coverage; the URML program, manifest, and validator "
    "are unchanged."
)
...
class AnymalAdapter:
    """ANYbotics ANYmal adapter (ROS 2), composing RclpyAdapter."""
    ...
    def __init__(
        ...
        *,
        node_name: str = "urml_anymal_quadruped",
    ...
    def __enter__(self) -> AnymalAdapter:
        return self
    ...
    # ------------------------------------------------------------------
    # Not supported on a bare ANYmal
    # ------------------------------------------------------------------
