"""Deployment-side configuration for ``OpcUaAdapter``.

An OPC UA Robotics cell exposes its capabilities as method and variable
...
program, manifest, or envelope — they live in an ``opcua_adapter.yaml``
...
class MethodTarget(BaseModel):
    """A node-id of an OPC UA method plus the literal args to call it with."""

    model_config = ConfigDict(extra="forbid")

    method: str = Field(..., description="NodeId of the method to call, e.g. 'ns=2;s=MoveTo'.")
    args: list[str] = Field(default_factory=list, description="Literal string args passed to the method.")


class OpcUaConfig(BaseModel):
    """OPC UA Robotics connection + node-mapping config."""

    model_config = ConfigDict(extra="forbid")

    endpoint_url: str = Field(
        default="opc.tcp://127.0.0.1:4840/urml/",
        description="OPC UA server endpoint. A local asyncua demo server uses opc.tcp://127.0.0.1:4840/.",
    )
...
def load_opcua_config(path: str | Path) -> OpcUaConfig:
    """Parse an ``opcua_adapter.yaml`` file into an ``OpcUaConfig``."""
    p = Path(path)
    with p.open(encoding="utf-8") as fh:
        data = yaml.safe_load(fh) or {}
    if not isinstance(data, dict):
        raise ValueError(f"opcua-config file {p} did not contain a YAML mapping at the top level.")
    return OpcUaConfig.model_validate(data)
