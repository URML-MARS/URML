"""OpcUaAdapter — OPC UA Robotics cell, via asyncua.

OPC UA Robotics is the universal factory-floor companion spec; this is
the industrial-automation analog of the PX4/marine no-ROS proof and
mirrors :class:`BlueRovAdapter`: lazy ``asyncua``, **no ROS 2
...
Supported (mapped onto configured OPC UA method/variable nodes):

- ``move_to`` / ``hover`` → call the configured per-location method
  node (location → method+args via :class:`OpcUaConfig`).
...
``not_applicable_opcua``.

A general "invoke a named ControlProgram / PLC method with arguments"
...
from urml_opcua_runtime.config import MethodTarget, OpcUaConfig, load_opcua_config
...
_NOT_SUPPORTED = (
    "not_supported_on_opcua_cell: a bare OPC UA Robotics cell has no {capability}. "
    "Pair it with a vision/HMI companion adapter; the URML program, manifest, "
    "and validator are unchanged."
)
_NOT_APPLICABLE = "not_applicable_opcua: {capability} has no meaning for a fixed industrial cell."


def _require_asyncua() -> Any:
    """Lazy-import asyncua's sync client with a clear error when missing."""
    try:
        from asyncua.sync import Client  # type: ignore[import-not-found,unused-ignore]
    except ImportError as exc:
        raise RuntimeError(
            "asyncua is not installed. OpcUaAdapter requires the [opcua] extra.\n"
            "  Install with: pip install urml-opcua-runtime[opcua]"
        ) from exc
    return Client


class OpcUaAdapter:
    """OPC UA Robotics adapter implementing the URML ROSAdapter Protocol."""

    BRAND = "opcua"

    def __init__(self, config: OpcUaConfig | None = None) -> None:
        self._client_cls = _require_asyncua()
    ...
    def __enter__(self) -> OpcUaAdapter:
        return self
        ...
        *,
        ...
        if location is None:
            return NavigationResult(
                success=False,
                reason="opcua_requires_named_location: an OPC UA cell drives to named "
        ...
        if mt is None:
            return NavigationResult(
                ...
                "manifest but not mapped to a method in opcua_adapter.yaml.",
            )
        self._call(mt)
    ...
    def send_docking_goal(self, *, station: str, service: str, until: str | None = None) -> NavigationResult:
        ...
        if mt is None:
            return NavigationResult(
                ...
                f"{station!r}) is not mapped to a method in opcua_adapter.yaml.",
            )
        self._call(mt)
        ...
        *,
        ...
        if mt is None:
            return ManipulationResult(
                ...
                "to a method in opcua_adapter.yaml.",
            )
        self._call(mt)
